      ...
      yAxes: [
        {
          ticks: {
            ...
            maxTicksLimit: 11,
            //Step 6 Adjust the logarithmic scale so the y axis shows only powers of 10.
...
//Step 5 (AJAX call to api.covid19api.com/summary)
function loadContent() {
  let today = dayjs().format("MM-DD-YYYY");
  let yesterday = dayjs().diff(60 * 60 * 24 * 1000);
  if (
    !localStorage.getItem("date") ||
    localStorage.getItem("date") < yesterday
  ) {
...
function fillChart() {
  ...
  for (let c of covidJsObj.Countries) {
    //Step 4 (countries with at least 50000 deaths)
    if (c.TotalDeaths > 50000) {
      //Step 2 object to create an array of objects containing Slug, TotalConfirmed, TotalDeaths, Population, and TotalConfirmedPer100000
      newConfirmedOver1000.push({
        ...
        TotalConfirmed100000: (100000 * c.TotalConfirmed) / populations[c.Slug]
      });
    }
  }
  //Step 3 sort data on TotalConfirmedPer100000, descending, so the graph shows the country with the highest TotalConfirmedPer100000 first
  newConfirmedOver1000 = _.orderBy(
    newConfirmedOver1000,
    ["TotalConfirmed100000"],
    ["desc"]
  );
  //Step 4 Show bars for TotalConfirmed, TotalDeaths, and TotalConfirmedPer100000in the graph
  ...
  chartData.data.datasets[2].backgroundColor = "rgba(137,196,244,1)"; //blue
